"""ARCHIVED from tt-metal branch kmabee/mistral4-prefill, 2026-08-19. Not in-tree.

Single-shot prefill profiler for Mistral Small 4 on a 32-chip Blackhole Galaxy. Kept for
reference only -- it was deliberately NOT included in the Mistral enablement PR, because:

  * The prefill runner ALWAYS chunks. prefill_producer.py:534 computes
    `target_chunks = (real_len + CHUNK_SIZE - 1) // CHUNK_SIZE` unconditionally, with
    CHUNK_SIZE defaulting to 5120 -- there is no single-shot branch. So single-shot numbers
    measure a code path production never takes.
  * The in-tree chunked test (test_prefill_transformer_chunked.py::
    test_mistral4_prefill_transformer_chunked_no_pcc) already prints per-chunk timings, so it
    doubles as the benchmark for the path that IS used.

Historical single-shot figures it produced on the 8 kW box (bh-glx-110-a10u08), for the record:
5,120 -> 26,203 tok/s; 25,600 -> 33,552 tok/s (the peak; single-shot dies on L1 at 102,400 with
"circular buffers grow to 1721216 B beyond max L1 size of 1572864 B"). Compare the production
chunked path at the same windows: 32,611 and 27,090 tok/s.

To run it again you must also restore the `serve_hook` parameter on run_model() in
tests/test_prefill_transformer.py -- this script calls run_model(..., serve_hook=hook) and that
parameter came from the interactive-server commit, which was also dropped.
"""
